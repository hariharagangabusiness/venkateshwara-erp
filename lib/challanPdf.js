...
async function generateChallanPdf(challan, items) {
  ...
  if (!browserPath) {
    throw new Error(
      'No Chrome/Edge/Chromium browser found on this machine. Challan PDFs are generated using your ' +
  ...
  try {
    ...
    await page.setContent(challanHtml(challan, items), { waitUntil: 'load' });
