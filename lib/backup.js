...
async function runBackup({ triggerType, triggeredBy } = {}) {
  ...
  try {
    ...
    // plain recursive copy, since there's no live-write-safety concern
    // here the way there is for the DB file.
    ...
    purgeOldBackups();
    return { ok: true, id: runId, artifactPath, totalSize, emailed: !!emailed, emailError, zohoUploaded: !!zohoUploaded, zohoError };
  } catch (e) {
