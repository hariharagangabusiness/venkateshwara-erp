...
// getOfferPdfTemplate()) - when its cover piece is both uploaded and
// switched on, it replaces this whole collage with a single full-page
// image; otherwise (the default for every installation until an admin
// explicitly opts in) this renders exactly as it always has.
function coverPage(template) {
...
//
// Same override rule as coverPage() above: only replaced when an admin has
// both uploaded an image and switched this piece on.
...
function bodyHtml(offer, client, items, techSpecs, boughtOut, terms, company, template) {
...
async function generateOfferPdf(offer, client, items, techSpecs, boughtOut, terms) {
  ...
  try {
    ...
    await page.setContent(bodyHtml(offer, client, items, techSpecs, boughtOut, terms, getCompanySettings(), template), { waitUntil: 'load' });
    await page.pdf({
      path: outPath,
      format: 'A4',
