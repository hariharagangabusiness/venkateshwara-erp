...
const { getCompanySettings, getOfferPdfTemplate } = require('./settings');
...
// Sized by max-height (not width) so the whole stack is guaranteed to fit
// within one A4 page's usable height alongside the header/footer margins,
// rather than by guessing widths and hoping the aspect ratios add up.
//
...
// supplied and switched on: uploaded Word cover page > uploaded image cover
// > the hand-tuned default collage below - each falls through to the next
// if its own content can't be read/parsed, so a bad upload never blanks the
// cover page outright.
async function coverPage(template) {
  ...
  <div class="cover-page">
    <div style="text-align:center;margin-bottom:10px;"><img src="${COVER_IMAGES.industries}" style="max-height:70px;"></div>
    <div style="text-align:center;margin-bottom:10px;"><img src="${COVER_IMAGES.fertilizerLine}" style="max-height:60px;"></div>
    <table style="border:none;margin-bottom:10px;"><tr>
      <td style="border:none;width:50%;padding:0 7px 0 0;text-align:center;"><img src="${COVER_IMAGES.factorySite}" style="max-height:130px;"></td>
      <td style="border:none;width:50%;padding:0 0 0 7px;text-align:center;"><img src="${COVER_IMAGES.logo}" style="max-height:100px;"></td>
    </tr></table>
    <div style="text-align:center;margin-bottom:10px;"><img src="${COVER_IMAGES.bagStation}" style="max-height:130px;"></div>
    <table style="border:none;"><tr>
      <td style="border:none;width:50%;padding:0 7px 0 0;text-align:center;"><img src="${COVER_IMAGES.conveyorSingle}" style="max-height:100px;max-width:100%;"></td>
      <td style="border:none;width:50%;padding:0 0 0 7px;text-align:center;"><img src="${COVER_IMAGES.conveyorMulti}" style="max-height:100px;max-width:100%;"></td>
    </tr></table>
  </div>
  <div class="page-break"></div>`;
}
...
function headerTemplate(template) {
  if (template && template.header_richtext_active && template.header_richtext) {
    return `<div style="width:100%;margin:0 15mm;font-family:'Calibri',Arial,sans-serif;font-size:10px;-webkit-print-color-adjust:exact;">${template.header_richtext}</div>`;
  }
  if (template && template.header_active && template.header_image_path) {
    const uri = overrideImageDataUri(template.header_image_path);
    if (uri) return `<div style="width:100%;margin:0 15mm;"><img src="${uri}" style="width:100%;display:block;"></div>`;
  }
  return `<div style="width:100%;font-size:9px;margin:0 15mm;font-family:'Times New Roman',Georgia,serif;-webkit-print-color-adjust:exact;">
    <div style="color:#0000FF;font-size:34.5px;letter-spacing:0.5px;text-align:center;margin:0;line-height:1.2;">VENKATESHWARA ENGINEERS</div>
    <div style="color:#808080;text-align:center;font-size:13.3px;font-weight:bold;letter-spacing:0.5px;margin:2px 0 6px;font-family:'Times New Roman',serif;">MANUFACTURERS OF MATERIAL HANDLING EQUIPMENTS</div>
    <div style="border-top:3px solid #000;"></div>
  </div>`;
}

function footerTemplate(template) {
  if (template && template.footer_richtext_active && template.footer_richtext) {
    return `<div style="width:100%;margin:0 15mm;font-family:'Calibri',Arial,sans-serif;font-size:10px;-webkit-print-color-adjust:exact;">${template.footer_richtext}</div>`;
  }
  if (template && template.footer_active && template.footer_image_path) {
    const uri = overrideImageDataUri(template.footer_image_path);
    if (uri) return `<div style="width:100%;margin:0 15mm;"><img src="${uri}" style="width:100%;display:block;"></div>`;
  }
  return `<div style="width:100%;font-size:13.3px;margin:0 15mm;text-align:center;font-family:'Times New Roman',serif;color:#000;-webkit-print-color-adjust:exact;">
...
const BASE_CSS = `
  * { box-sizing: border-box; }
  ...
  body { font-family: 'Calibri', Arial, sans-serif; font-size: 13.3px; line-height: 1.5; color: #111; margin: 0; }
  p { margin: 0 0 12px; }
  h2.section-title, h3.sub-title { font-size: 13.3px; font-weight: bold; text-decoration: underline; color: #111; margin: 0 0 10px; }
...
async function bodyHtml(offer, client, items, techSpecs, boughtOut, terms, company, template) {
...
async function generateOfferPdf(offer, client, items, techSpecs, boughtOut, terms) {
  ...
  try {
    ...
    await page.setContent(await bodyHtml(offer, client, items, techSpecs, boughtOut, terms, getCompanySettings(), template), { waitUntil: 'load' });
    await page.pdf({
      path: outPath,
      format: 'A4',
...
module.exports = { generateOfferPdf, bodyHtml, coverPage, headerTemplate, footerTemplate };
