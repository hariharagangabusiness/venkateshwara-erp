...
const { getCompanySettings, getOfferPdfTemplate, getOfferDesignTokens, getOfferPdfLayout } = require('./settings');
...
// > uploaded PDF cover > uploaded Word cover page > uploaded image cover >
// the hand-tuned default collage below - each falls through to the next if
// its own content can't be read/parsed/is inactive, so a bad upload never
// blanks the cover page outright.
//
...
function headerTemplate(template, tokens) {
  ...
  } else if (template && template.header_richtext_active && template.header_richtext) {
...
function footerTemplate(template, tokens) {
  ...
  } else if (template && template.footer_richtext_active && template.footer_richtext) {
...
async function generateOfferPdf(offer, client, items, techSpecs, boughtOut, terms) {
  ...
  // per-page stamp below; offer/client are already in scope here. Also
  // carries pdfLayout/mergeTokenMap through, since headerTemplate()/
  // footerTemplate()/coverPage() only ever receive `tokens`, not a separate
  // parameter for each.
  const tokens = {
  ...
  // Uploaded PDF cover (second-highest-precedence cover source, after a
  // designed cover from the Layout Designer - see coverPage()) isn't part of
  // the HTML Puppeteer just rendered, so it's prepended here as a real
  // page-level merge instead. Uses the exact same existence/signature check
  // coverPage() already used to decide to render nothing, so the two can
  // never disagree about whether a cover was supplied. Skipped entirely when
  // a designed cover is active - coverPage() already rendered that cover
  // inline, so merging the uploaded PDF on top of it too would duplicate the
  // cover. A failure at this point (e.g. a corrupt file that still passed
  // the signature check) is logged and swallowed rather than failing the
  // whole download - the offer PDF still comes out, just without its cover.
  const layoutCoverActive = layout.cover.active && layout.cover.html;
  const coverPdfAbs = (!layoutCoverActive && template.cover_pdf_active) ? pdfCoverAbsPath(template.cover_pdf_path) : null;
  if (coverPdfAbs) {
    try {
