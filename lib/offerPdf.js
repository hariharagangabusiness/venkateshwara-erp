...
//
// Multi-image rows use flexbox (justify-content:space-between) with each
// <img> sized by its own max-height/max-width rather than the old approach
// of centering an image inside a 50%-wide table cell - centering a narrow
// image within a half-page-wide cell left large dead-air gaps between
// images that were meant to sit close together (the "pictures are all over
// the place" bug). Each row's height is capped independently, and the sum
// stays well inside one A4 page's usable height, so a page-break is still
// only ever needed at the end of this section, not partway through it.
//
...
async function coverPage(template) {
  ...
  <div class="cover-page">
    <div style="text-align:center;margin-bottom:12px;"><img src="${COVER_IMAGES.industries}" style="max-width:100%;max-height:90px;"></div>
    <div style="text-align:center;margin-bottom:12px;"><img src="${COVER_IMAGES.fertilizerLine}" style="max-width:100%;max-height:80px;"></div>
    ...
    <div style="text-align:center;"><img src="${COVER_IMAGES.productSwatches}" style="max-width:100%;max-height:75px;"></div>
  </div>
  <div class="page-break"></div>`;
}
