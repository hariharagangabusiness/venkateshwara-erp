...
const { getCompanySettings, getOfferPdfTemplate, DEFAULT_OFFER_DESIGN_TOKENS, getOfferPdfLayout } = require('./settings');
...
// Replaces {{token.name}} placeholders (see generateOfferPdf()'s
// mergeTokenMap) inside a piece designed in the Offer PDF Layout Designer -
// the only place free-form {{...}} text can come from is that Admin-only
// canvas, so this runs unconditionally rather than behind an opt-in flag.
// An unrecognized token (a typo, or a field this version doesn't support
// yet) is left as literal {{...}} text rather than silently vanishing, so
// the mistake is obvious in the rendered PDF instead of hidden.
function substituteTokens(html, tokenMap) {
  return String(html || '').replace(/\{\{\s*([\w.]+)\s*\}\}/g, (m, key) => {
    const v = tokenMap ? tokenMap[key] : undefined;
    return (v !== undefined && v !== null) ? esc(String(v)) : m;
  });
}

const COMPANY_ADDRESS_LINES = [
  `222,TYPE 'D;' 221,TYPE 'D;'215,TYPE 'B' &amp; PLOT: 123 &amp; 124;HSIIDC; SECTOR-59`,
...
// supplied and switched on: a cover designed in the Offer PDF Layout
// Designer (lib/settings.js's getOfferPdfLayout(), the newer visual builder)
// > uploaded image cover > the hand-tuned default collage below - each
// falls through to the next if its own content can't be read/is inactive,
// so a bad upload never blanks the cover page outright.
async function coverPage(template, tokens) {
  const layout = tokens && tokens.pdfLayout;
  if (layout && layout.cover && layout.cover.active && layout.cover.html) {
    const html = substituteTokens(layout.cover.html, tokens.mergeTokenMap);
    return `
    <div class="cover-page"><style>${layout.cover.css || ''}</style>${html}</div>
...
//
// Precedence when more than one piece is supplied and switched on: a header
// designed in the Offer PDF Layout Designer > an uploaded image > the
// hand-tuned default below, driven by lib/settings.js's
// DEFAULT_OFFER_DESIGN_TOKENS (a fixed constant, not admin-editable) - every
// value matches what used to be hardcoded here exactly, so this renders
// byte-for-byte the same as the original letterhead.
function headerTemplate(template, tokens) {
  const t = tokens || {};
  const layout = t.pdfLayout;
  let core;
  if (layout && layout.header && layout.header.active && layout.header.html) {
    // Puppeteer's header/footer template context is its own tiny mini-
    // document - it does NOT inherit baseCss(tokens) the way the main body
    // does, and its unset default font-size renders at a barely-visible
    // couple of pixels. This wrapper gives designed content a sane default
    // to start from; anything the designer explicitly styled still wins.
    core = `<style>${layout.header.css || ''}</style><div style="font-size:10px;font-family:Arial,sans-serif;">${substituteTokens(layout.header.html, t.mergeTokenMap)}</div>`;
  } else {
...
function footerTemplate(template, tokens) {
  ...
  const layout = t.pdfLayout;
  let core;
  if (layout && layout.footer && layout.footer.active && layout.footer.html) {
    // Same tiny-default-font caveat as headerTemplate()'s equivalent branch.
    core = `<style>${layout.footer.css || ''}</style><div style="font-size:10px;font-family:Arial,sans-serif;">${substituteTokens(layout.footer.html, t.mergeTokenMap)}</div>`;
  } else {
    ...
    if (overrideUri) {
      ...
      core = `<div style="width:100%;font-size:${t.footer_font_size_px}px;margin:0 15mm;text-align:center;font-family:'Times New Roman',serif;color:${t.footer_color};-webkit-print-color-adjust:exact;">
        ...
        <!-- white-space:nowrap - confirmed via a canvas measureText probe that
             each line comfortably fits the footer's actual width (a few
             hundred px against ~680-790px available), yet without this the
             Chromium print pipeline still wraps every line into two and
             renders each half centered independently, producing the jagged,
             off-center look this was reported for. Same class of
             print-template-only quirk as the header/footer CSS-inheritance
             note above - confirmed by direct render, not by spec. -->
        <div style="font-weight:bold;line-height:1.4;white-space:nowrap;">${COMPANY_ADDRESS_LINES.join('<br>')}</div>
      </div>`;
    }
  }
...
// Built from lib/settings.js's DEFAULT_OFFER_DESIGN_TOKENS (a fixed
// constant, not admin-editable) - every value below matches what used to be
// hardcoded here exactly, so this renders byte-for-byte the same letterhead.
function baseCss(tokens) {
  ...
  * { box-sizing: border-box; }
  ...
  /* border-collapse:collapse stops Chromium repeating <thead> across a page
     break when printing, so cell borders are drawn with box-shadow instead
     of border - same look, doesn't trigger the bug. */
  table { width: 100%; border-collapse: separate; border-spacing: 0; margin-bottom: 12px; table-layout: fixed; }
  thead { display: table-header-group; }
  tr { page-break-inside: avoid; }
  th, td { box-shadow: inset 0 0 0 1px #333; padding: 4px 6px; font-size: 12px; text-align: left; vertical-align: top; word-wrap: break-word; overflow-wrap: break-word; }
...
// Decides how a picked item's picture and summary blurb share the Equipment
// Description page, based on the picture's own aspect ratio rather than a
// fixed layout for every item: a wide/landscape photo reads better with its
// summary stacked underneath (squeezing it into a side column would shrink
// the photo too much to be useful), while a narrower portrait or roughly
// square photo leaves enough horizontal room to sit beside the text
// instead, so the two share the page more efficiently. Falls back to the
// original simple stacked layout - no summary paragraph at all - for an
// item with no summary text, or whose image can't be measured (corrupt or
// unrecognized data), so an offer with no summaries set renders exactly as
// it always has.
const LANDSCAPE_ASPECT_THRESHOLD = 1.2;
function equipmentDescriptionBlock(it) {
  const title = esc(it.section_title || it.description || '');
  const summaryHtml = it.summary ? nl2br(it.summary) : '';
  if (!summaryHtml) {
  ...
  let dimensions = null;
  try {
    const base64 = String(it.image_data_uri).split(',')[1] || '';
    dimensions = imageSize(Buffer.from(base64, 'base64'));
  } catch (e) { /* unrecognized/corrupt image data - stacked layout below is a safe default */ }
  const isLandscape = !dimensions || (dimensions.width / dimensions.height) >= LANDSCAPE_ASPECT_THRESHOLD;

  if (isLandscape) {
    return `<div class="box">
      <p><b>${title}</b></p>
      <img src="${it.image_data_uri}" style="max-width:100%;max-height:280px;display:block;margin:0 auto;">
      <p style="margin-top:8px;">${summaryHtml}</p>
    </div>`;
  }
  return `<div class="box" style="display:flex;flex-wrap:wrap;gap:14px;align-items:flex-start;">
    <img src="${it.image_data_uri}" style="max-width:45%;max-height:280px;flex:0 0 auto;">
...
async function bodyHtml(offer, client, items, techSpecs, boughtOut, terms, company, template, tokens) {
  ...
  const imageBlocks = items.filter(it => it.image_data_uri).map(equipmentDescriptionBlock).join('');
  ...
  ${await coverPage(template, tokens)}
  ...
  ${showTechSpecs ? `<table><colgroup><col style="width:42%"><col style="width:58%"></colgroup><thead><tr><th colspan="2">Specifications</th></tr></thead><tbody>${specRows}</tbody></table>` : ''}
  ...
  <table>
    ...
    <thead><tr><th>Item</th><th>Description</th><th>Qty</th><th>Unit Price (Ex Works)</th><th>Total Price</th></tr></thead>
    <tbody>
    ${itemRows}
...
async function generateOfferPdf(offer, client, items, techSpecs, boughtOut, terms) {
  ...
  // Optional visual layout (lib/settings.js's getOfferPdfLayout()) - the
  // newer, higher-precedence alternative to `template` above. Each of its
  // three pieces defaults to inactive, so an installation that has never
  // opened the Offer PDF Layout Designer renders exactly as before.
  const layout = getOfferPdfLayout();
  const offerDateStr = new Date(offer.offer_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
  // Flat {{token}} -> value map for substituteTokens() - the fields a piece
  // designed in the Layout Designer can reference. Keep in sync with the
  // token picker list public/js/app.js shows in that page.
  const mergeTokenMap = {
    'client.name': client.name,
    'offer.no': offer.offer_no,
    'offer.subject': offer.subject,
    'offer.date': offerDateStr,
    'offer.version': offer.version ? 'v' + offer.version : '',
    'offer.application': offer.application,
    'offer.type_of_system': offer.type_of_system,
    'offer.material_of_construction': offer.material_of_construction,
    'company.legal_name': company.legal_name,
    'company.registered_address': company.registered_address,
    'company.gstin': company.gstin,
    'company.pan': company.pan,
  };
  // Fixed design tokens (lib/settings.js's DEFAULT_OFFER_DESIGN_TOKENS, not
  // admin-editable), plus pdfLayout/mergeTokenMap for the Layout Designer -
  // headerTemplate()/footerTemplate()/coverPage() only ever receive
  // `tokens`, not a separate parameter for each.
  const tokens = {
  ...
  try {
    ...
    await page.setContent(await bodyHtml(offer, client, items, techSpecs, boughtOut, terms, company, template, tokens), { waitUntil: 'load' });
    await page.pdf({
      path: outPath,
      format: 'A4',
...
module.exports = { generateOfferPdf, bodyHtml, coverPage, headerTemplate, footerTemplate, baseCss };
