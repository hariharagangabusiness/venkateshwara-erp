...
function poHtml(po, vendor, company, companyAddress) {
  ...
  ${documentStampHtml({
    ...
    reference: po.po_no,
    partyLabel: 'Vendor',
    partyName: vendor.legal_name || vendor.name,
...
async function generatePoPdf(po, vendor, company, companyAddress) {
  ...
  try {
    ...
    await page.setContent(poHtml(po, vendor, company, companyAddress), { waitUntil: 'load' });
