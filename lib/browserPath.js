...
// Dockerfile, an apt package that happens to expose one on PATH, or a dev
// machine with chromium-browser installed). NOT how the Railway fix works:
// two earlier attempts tried installing a system Chromium (first via a Nix
// package, then via apt) on the theory that it would show up here, but
// neither ever actually ran - the config file both attempts used
// (nixpacks.toml) is for Railway's Nixpacks builder, and this project's
// Railway service actually builds with Railpack, which never reads that
// file at all (confirmed from a build log showing `load build definition
// from ./railpack-plan.json`, not any Nixpacks output). See railpack.json
// at the repo root, which installs the missing shared libraries via
// Railpack's own deploy.aptPackages - that fixes @sparticuz/chromium's own
// bundled binary directly (see getLaunchOptions() below) without this PATH
// lookup needing to find anything.
function findOnPath() {
