...
function ensureEditableVersion(offerId, userId, reason) {
  ...
  if (!full) { const err = new Error('Offer not found'); err.status = 404; throw err; }
  const { offer } = full;

  // Round 40: an offer locked by SO conversion (or an Admin-only manual
  // lock) can't even be forked into a new revision - unlike an ordinary
  // Sent/Won-but-unlocked offer, which forks freely. This is the app-level
  // half of the guard; schema.sql's triggers are the database-level
  // backstop if this check is ever bypassed.
  if (offer.locked) {
    const err = new Error('This offer has been converted to a Sales Order and is locked against further edits. Start a new offer for any further revision, or ask an Admin to unlock it first.');
    err.status = 423;
    throw err;
  }
