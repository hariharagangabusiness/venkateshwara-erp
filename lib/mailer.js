...
// fromName/fromAddress optionally override the global identity (e.g. a
// department's own "From" - see lib/departmentEmail.js) while still relaying
// through the one shared SMTP account above. The mail server has to actually
// accept sending as that address (an alias/mailbox it recognizes) or it may
// reject the send or silently rewrite the From header - this override just
// sets what the app asks for, it can't itself grant that authorization.
async function sendMail({ to, cc, subject, text, html, attachments, fromName, fromAddress }) {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"${fromName || cfg.fromName}" <${fromAddress || cfg.fromAddress}>`,
